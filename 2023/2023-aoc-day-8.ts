...
class Day8_2023 extends Solution {
    ...
    partOne(input: string[]): any {
        ...
        return this.findZCycleLength(pattern, routes, "AAA", (place) => place === "ZZZ");
    }
     ...
     */
    findZCycleLength(pattern: string, routes: {}, start: string, isFinished: (place: string) => boolean) {
        let numMoves = 0;
        let curPlace = start;
        let pIndex = 0;
    ...
    partTwo(input: string[]): any {
        ...
        for (let place in routes) {
            ...
            if (place.charAt(2) === "A") {
                let cLength = this.findZCycleLength(pattern, routes, place, (p) => p.charAt(2) === "Z");
